...
def system(y, t, a, b, m, I, E, r, q, s, d):
    
    N, R, H, P = y
    dydt = [I - E*N + (r[0]*(m[0]*R)**q[0])/(s[0]**q[0]+(m[0]*R)**q[0])*(t-d[0]) + (r[0]*(m[1]*N)**q[0])/(s[0]**q[0]+(m[1]*N)**q[0])*(t-d[0]) + (r[0]*(m[2]*P)**q[0])/(s[0]**q[0]+(m[2]*P)**q[0])*(t-d[0]) - a[0]*N*R/(b[0] + N),
...
solved = odeint(system, y0, trange, args = (a, b, m, I, E, r, q, s, d), atol = 1.49012e-10)
...
qmin = 0
qmax = 1.5
q_range = float_range(qmin, qmax, '0.01') #range of r values to be plotted
...
for var in r_range: #iterating through different instances of q and r
    
    iter_r = [var]
    solved = odeint(system, y0, trange, args = (a, b, m, I, E, iter_r, q, s), atol = 1.49012e-10)
    ...
    equilibrium_values = (sy.nsolve([I - E*N + (r[0]*(m[0]*R)**q[0])/(s[0]**q[0]+(m[0]*R)**q[0]) + (r[0]*(m[1]*H)**q[0])/(s[0]**q[0]+(m[1]*H)**q[0]) + (r[0]*(m[2]*P)**q[0])/(s[0]**q[0]+(m[2]*P)**q[0]) - a[0]*N*R/(b[0] + N),
    ...
    J = np.array([[-(a[0]*R)/(b[0]+N)+(a[0]*R*N)/(b[0]+N)**2-E,
                   (np.power(s[0], q[0])*q[0]*iter_r[0]*np.power((m[0]*R), q[0]))/(R*(np.power((m[0]*R), q[0])+np.power(s[0],q[0]))**2),
                   (np.power(s[0], q[0])*q[0]*iter_r[0]*np.power((m[1]*H), q[0]))/(H*(np.power((m[1]*H), q[0])+np.power(s[0], q[0]))**2),
                   (np.power(s[0], q[0])*q[0]*iter_r[0]*np.power((m[2]*P), q[0]))/(R*(np.power((m[2]*P), q[0])+np.power(s[0], q[0]))**2)],
                  
                  [(a[0]*b[0]*R)/(b[0]+N)**2,
                   -(a[1]*H)/(b[1]+R) + (a[1]*H*R)/(b[1]+R)**2 + (a[0]*N)/(b[0]+N)-m[0],
                   (a[1]*R)/(b[1]+R),
                   0],
                  
                   [0, (a[1]*b[1]*H)/(b[1]+R)**2,
                    -(a[1]*P)/(b[2]+H) + (a[1]*H*P)/(b[2]+P)**2 + (a[1]*R)/(b[1]+R)-m[1],
                    -(a[1]*H)/(b[2]+H)],
